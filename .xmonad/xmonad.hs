...
myMouseBindings (XConfig {XMonad.modMask = modm}) =
  M.fromList
    ...
    [ ( (modm, button1),
        \w ->
          focus w >> Flex.mouseWindow Flex.position w
      ...
      ( (modm, button3),
        \w ->
          focus w >> Flex.mouseWindow Flex.resize w
            >> windows W.shiftMaster
      )
    ]
...
scratchpads =
  [ -- run htop in xterm, find it by title, use default floating window placement
    ...
    NS
      "ncpamixer"
      "st -n ncpamixer -e ncpamixer"
      (resource =? "ncpamixer")
      (customFloating $ W.RationalRect (1 / 4) (2 / 3) (2 / 4) (1 / 4)),
    NS
      "ncspot"
      "st -n ncspot -e ncspot"
      (resource =? "ncspot")
    ...
    NS
      "teams"
      "systemd-run --scope -p MemoryMax=8G -p CPUQuota=800% --user teams"
